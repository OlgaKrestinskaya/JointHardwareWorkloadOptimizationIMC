...
def run_several_iterationsJoint(dnn_names, num_generations, population_size, num_iterations_to_run, seed_alg1, areaConstrcm2,dir_noDNNname,obj_type,num_mapping,n_jobs,XbarXin, XbarYin, TrouterIN, TchipIN, CtileIN, GlbIN):
    if seed_alg1==None:
        seed_alg=None
    else:
        seed_alg=seed_alg1
    for testnumber in range(num_iterations_to_run):
        ...
        class MySamplingSeveralParallel(FloatRandomSampling):
            def _do(self, problem, n_samples, **kwargs):
                ...
                while len(final_output)<n_samples:
                    ...
                    for i in range(output.shape[0]):
                        for dnn_name in dnn_names:
                            params_ind=[int(output[i][0]), int(output[i][1]), crossbar_sizeX.index(XbarXin), crossbar_sizeY.index(XbarYin), shared_router_groupSize.index(TrouterIN), tiles_in_chip.index(TchipIN), macros_in_tile.index(CtileIN), glb_buffer_depth.index(GlbIN)]
        ...
        class MyProblemParallel(Problem):
            ...
            def __init__(self):
                ...
                super().__init__(n_var=2, n_obj=1, n_ieq_constr=1, xl=[0,0], xu=[7,5], vtype=int,elementwise_evaluation=True)
               ...
               # print(newxxx)
                ...
                for i in range(new_x.shape[0]):
                    ...
                    try:
                        ...
                        vl=new_x[i][0]
                        bl=new_x[i][1]
                        csX,csY,srg,tic,mit,gbp= XbarXin, XbarYin, TrouterIN, TchipIN, CtileIN, GlbIN
                        bps=1
                        vl,bl =[float(x) for x in [vl,bl]]
                        ...
                        ############################
                            ...
                            #for dnn_name in dnn_names:
                                #params_ind=[x[i][0], x[i][1], crossbar_sizeX.index(XbarXin), crossbar_sizeY.index(XbarYin), shared_router_groupSize.index(TrouterIN), tiles_in_chip.index(TchipIN), macros_in_tile.index(CtileIN), glb_buffer_depth.index(GlbIN)]
                                #check=checkSizeParallel(params_ind,dnn_name,layers_all[dnn_name],inp_bits,n_jobs)
                                #if check==False:
                            ...
                            if check==True: #if we reach the last check and it's true
                                ...
                                paramlist=[new_x[i][0], new_x[i][1], XbarXin, XbarYin, TrouterIN, TchipIN, CtileIN, GlbIN]
