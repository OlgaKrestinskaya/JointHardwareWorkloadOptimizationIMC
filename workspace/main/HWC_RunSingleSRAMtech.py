...
def run_layer(
    ...
    vl,bps,bl,csX,csY,srg,tic,mit,gbp, tech=params
    vl,bl =[float(x) for x in [vl,bl]]
    bps,csX,csY,srg,tic,mit,gbp, tech=[int(x) for x in [bps,csX,csY,srg,tic,mit,gbp, tech]]
    ...
    spec.variables.update(
        dict(
            ...
            TECHNOLOGY=tech,  # nm  --> adjust this
...
def runSingle(dnn_names, num_iterations_to_run, num_mapping,n_jobs, hardware_params):
    ...
    #layers = sorted(l.split(".")[0] for l in layers)
    resultsfile='DictMaxSRAMtech.json'
    if not os.path.exists(resultsfile):
        data = {}  # Empty dictionary
        with open(resultsfile, 'w') as file:
            json.dump(data, file, indent=4)

    vl,bps,bl,csX,csY,srg,tic,mit,gbp,tech=hardware_params
    vl,bl =[float(x) for x in [vl,bl]]
    write_to_dic=[int(x) for x in [bps,csX,csY,srg,tic,mit,gbp,tech]]
    bps,csX,csY,srg,tic,mit,gbp,tech=[int(x) for x in [bps,csX,csY,srg,tic,mit,gbp,tech]]
    #for 
    #write_to_dic=vl,bps,bl,csX,csY,srg,tic,mit,gbp
    write_to_dic=vl,bps,bl,csX,csY,srg,tic,mit,gbp,tech
