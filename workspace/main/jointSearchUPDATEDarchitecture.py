...
def run_several_iterationsJoint(dnn_names, num_generations, population_size, num_iterations_to_run, seed_alg1, areaConstrcm2,dir_noDNNname,obj_type,num_mapping,n_jobs,Vinp,BitsIn, base_latIN, XbarXin, XbarYin):
    if seed_alg1==None:
        seed_alg=None
    else:
        seed_alg=seed_alg1
    for testnumber in range(num_iterations_to_run):
        ...
        class MySamplingSeveralParallel(FloatRandomSampling):
            def _do(self, problem, n_samples, **kwargs):
                ...
                while len(final_output)<n_samples:
                    ...
                    for i in range(output.shape[0]):
                        for dnn_name in dnn_names:
                            params_ind=[voltages.index(Vinp), bits_per_cell.index(BitsIn), base_latency.index(base_latIN), crossbar_sizeX.index(XbarXin), crossbar_sizeY.index(XbarYin), int(output[i][0]), int(output[i][1]), int(output[i][2]), int(output[i][3])]
        ...
        class MyProblemParallel(Problem):
            ...
            def __init__(self):
                ...
                super().__init__(n_var=4, n_obj=1, n_ieq_constr=5, xl=[0,0,0,0], xu=[5,4,5,6], vtype=int,elementwise_evaluation=True)
            ...
            def _evaluate(self, x, out, *args, **kwargs):
               ...
               # print(newxxx)
                ...
                for i in range(x.shape[0]):
                    ...
                    new_x[i][0]=shared_router_groupSize[x[i][0]]
                    new_x[i][1]=tiles_in_chip[x[i][1]]
                    new_x[i][2]=macros_in_tile[x[i][2]]
                    new_x[i][3]=glb_buffer_depth[x[i][3]]
                ...
                for i in range(new_x.shape[0]):
                    ...
                    try:
                        ...
                        gbp=new_x[i][3]
                        #srg,tic,mit,gbp
                        vl,bps, bl,csX,csY= Vinp,BitsIn, base_latIN, XbarXin, XbarYin
                        ...
                        ############################
                            ...
                            for dnn_name in dnn_names:
                                params_ind=[voltages.index(Vinp), bits_per_cell.index(BitsIn), base_latency.index(base_latIN), crossbar_sizeX.index(XbarXin), crossbar_sizeY.index(XbarYin), x[i][0], x[i][1], x[i][2], x[i][3]]
                                check=checkSizeParallel(params_ind,dnn_name,layers_all[dnn_name],inp_bits,n_jobs)
                                if check==False:
                            ...
                            if check==True: #if we reach the last check and it's true
                                ...
                                paramlist=[Vinp,BitsIn, base_latIN, XbarXin, XbarYin, new_x[i][0], new_x[i][1], new_x[i][2], new_x[i][3]]
