...
def run_several_iterationsJoint(dnn_names, num_generations, population_size, num_iterations_to_run, seed_alg1, areaConstrcm2,dir_noDNNname,obj_type,num_mapping,n_jobs,Vinp, BitsIn, base_latIN, TrouterIN, TchipIN, CtileIN, GlbIN):
    if seed_alg1==None:
        seed_alg=None
    else:
        seed_alg=seed_alg1
    for testnumber in range(num_iterations_to_run):
        ...
        class MySamplingSeveralParallel(FloatRandomSampling):
            def _do(self, problem, n_samples, **kwargs):
                ...
                while len(final_output)<n_samples:
                    ...
                    for i in range(output.shape[0]):
                        for dnn_name in dnn_names:
                            params_ind=[voltages.index(Vinp), bits_per_cell.index(BitsIn), base_latency.index(base_latIN), int(output[i][0]), int(output[i][1]), shared_router_groupSize.index(TrouterIN), tiles_in_chip.index(TchipIN), macros_in_tile.index(CtileIN), glb_buffer_depth.index(GlbIN)]
                            #print(params_ind)
        ...
        class MyProblemParallel(Problem):
            ...
            def __init__(self):
                ...
                super().__init__(n_var=2, n_obj=1, n_ieq_constr=5, xl=[0,0], xu=[4,4], vtype=int,elementwise_evaluation=True)
            ...
            def _evaluate(self, x, out, *args, **kwargs):
               ...
               # print(newxxx)
                ...
                for i in range(x.shape[0]):
                    ...
                    #ew_x[i][1]=bits_per_cell[x[i][1]]
                    #new_x[i][2]=base_latency[x[i][2]]
                    new_x[i][0]=crossbar_sizeX[x[i][0]]
                    new_x[i][1]=crossbar_sizeY[x[i][1]]
                ...
                for i in range(new_x.shape[0]):
                    ...
                    try:
                        csX=new_x[i][0]
                        csY=new_x[i][1]
                        vl,bps,bl,srg,tic,mit,gbp= Vinp, BitsIn, base_latIN, TrouterIN, TchipIN, CtileIN, GlbIN
                        ...
                        ############################
                            ...
                            for dnn_name in dnn_names:
                                params_ind=[voltages.index(Vinp), bits_per_cell.index(BitsIn), base_latency.index(base_latIN), x[i][0], x[i][1], shared_router_groupSize.index(TrouterIN), tiles_in_chip.index(TchipIN), macros_in_tile.index(CtileIN), glb_buffer_depth.index(GlbIN)]
                                check=checkSizeParallel(params_ind,dnn_name,layers_all[dnn_name],inp_bits,n_jobs)
                                if check==False:
                            ...
                            if check==True: #if we reach the last check and it's true
                                ...
                                paramlist=[Vinp,BitsIn, base_latIN, new_x[i][0], new_x[i][1], TrouterIN, TchipIN, CtileIN, GlbIN]
